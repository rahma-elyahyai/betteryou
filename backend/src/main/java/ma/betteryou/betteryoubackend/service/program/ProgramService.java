...
public class ProgramService {
    ...
    public List<ProgramCardDTO> getProgramsForUser(Integer userId) {
        List<WorkoutProgramR> programs = programRepository.findActiveProgramsByUserId(userId);
        return programs.stream()
                .map(this::mapToDto)
                .toList();
    }

    private ProgramCardDTO mapToDto(WorkoutProgramR p) {
        ...
        List<WorkoutSessionR> sessions = p.getSessions();
        ...
        int totalMinutes = sessions == null ? 0 :
                sessions.stream()
                        ...
                        .mapToInt(WorkoutSessionR::getDurationMinutes)
                        .sum();
        ...
        dto.setNextSessionDate(
                sessions == null ? null :
                        sessions.stream()
                                ...
                                .map(WorkoutSessionR::getSessionDate)
        ...
        dto.setRecentSessions(
                sessions == null ? List.of() :
                        sessions.stream()
                                .sorted(Comparator.comparing(WorkoutSessionR::getSessionDate,
