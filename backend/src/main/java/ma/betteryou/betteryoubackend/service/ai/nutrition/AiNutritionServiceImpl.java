...
import org.springframework.beans.factory.annotation.Value;
    protected NutritionPlan persistAll(AiNutritionGenerateRequest req, User user, AiNutritionAiResponse ai) {
        ...
        NutritionPlan plan = NutritionPlan.builder()
                .nutritionName(ai.getNutritionName() != null && !ai.getNutritionName().isBlank()
                        ? ai.getNutritionName()
        ...
        for (AiNutritionAiResponse.AiDay day : ai.getDays()) {
            ...
            for (AiNutritionAiResponse.AiMeal m : day.getMeals()) {
                // persist each meal and its relations in a separate transaction to avoid long single transactions
    ...
    public List<AiDayPlanDto> getWeek(Long planId) {
        ...
        Map<String, List<ComposedOf>> byDay = links.stream().collect(Collectors.groupingBy(c -> c.getId().getDayOfWeek()));
    ...
    protected void saveMealTransactional(NutritionPlan plan, String dow, AiNutritionAiResponse.AiMeal m) {
        ...
        Meal meal = mealRepository.findById(mealId).orElseThrow(() -> new RuntimeException("Inserted meal not found: " + mealId));
        ...
        if (m.getItems() != null) {
            for (AiNutritionAiResponse.AiItem it : m.getItems()) {
                ...
                containsRepository.save(Contains.builder()
                        ...
                        .meal(meal)
                        .foodItem(food)
                        .quantityGrams(it.getQuantityGrams() == null ? BigDecimal.valueOf(100) : BigDecimal.valueOf(it.getQuantityGrams()))
                        .build()
                );
            }
        }
        ...
        FoodItem food = foodRepository.findAll().stream()
                ...
                .orElseGet(() ->
                        foodRepository.save(FoodItem.builder()
                                .foodName(it.getFoodName())
                                .description("AI generated")
                                .caloriesPer100g(BigDecimal.valueOf(100)) // valeur par défaut
                                .proteinsPer100g(BigDecimal.valueOf(5))
                                .carbsPer100g(BigDecimal.valueOf(10))
                                .fatsPer100g(BigDecimal.valueOf(5))
                                .build()
...
public class AiNutritionServiceImpl implements AiNutritionService {
    ...
    // persistAll no longer holds a long transaction; we delegate each meal to a REQUIRES_NEW transaction
    protected NutritionPlan persistAll(AiNutritionGenerateRequest req, User user, AiNutritionAiResponse ai) {
