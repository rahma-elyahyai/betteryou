...
export default function NutritionAiWizard() {
  ...
  // ✅ VÉRIFICATION 1 : Chargement en cours
  if (loadingUser) {
    return (
      <div className="flex min-h-screen bg-black">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="flex flex-col items-center">
            <div className="w-16 h-16 border-4 border-gray-800 border-t-[#D6F93D] rounded-full animate-spin"></div>
            <p className="mt-4 text-[#D6F93D] font-semibold">Loading...</p>
          </div>
        </div>
      </div>
    );
  ...
  // ✅ VÉRIFICATION 2 : Utilisateur non connecté
  if (!user) {
    return (
      <div className="flex min-h-screen bg-black">
        <Sidebar />
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="text-center">
            ...
            <p className="text-white/50 mb-6">You must be logged in to generate nutrition plans.</p>
            <button
              onClick={() => window.location.href = '/login'}
              className="px-8 py-3 bg-[#D6F93D] text-black font-bold rounded-2xl hover:scale-105 transition"
            >
              Go to Login
            </button>
          </div>
  ...
  // ✅ VÉRIFICATION 3 : ID utilisateur manquant
  if (!user.idUser) {
    console.error("User object exists but idUser is missing:", user);
    return (
      <div className="flex min-h-screen bg-black">
        <Sidebar />
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="text-center">
            ...
            <p className="text-white/50 mb-6">Please try logging in again</p>
            <button 
              onClick={() => window.location.href = '/login'}
              className="px-6 py-3 bg-[#D6F93D] text-black font-semibold rounded-xl hover:bg-lime-500 transition"
            >
              Go to Login
            </button>
          </div>
  ...
  // ✅ Tout est OK, afficher le contenu principal
  return <NutritionAiWizardContent user={user} />;
}

// ===============================================
// ✅ COMPOSANT PRINCIPAL (après vérifications)
// ===============================================
function NutritionAiWizardContent({ user }) {
  ...
  async function handleGenerate(e) {
    ...
    // ✅ Utiliser user.idUser (déjà vérifié)
    ...
    try {
      ...
      console.log("✅ Response from backend:", gen);
      ...
      if (days?.length) {
        ...
        throw new Error("No days returned from backend.");
      }
              ...
              <div className="rounded-[2.5rem] border border-white/10 bg-white/5 p-10 backdrop-blur-xl shadow-2xl">
                ...
                <form onSubmit={handleGenerate} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="group">
                      <label className="block text-white/50 text-xs font-bold uppercase tracking-widest mb-2.5 group-focus-within:text-[#D6F93D]">
                        Commencement Date
                      </label>
                      <input
                        type="date"
                    ...
                    <div className="group">
                      <label className="block text-white/50 text-xs font-bold uppercase tracking-widest mb-2.5">
                        Completion Date
                      </label>
            ...
            <div className="space-y-10 animate-in fade-in duration-1000">
              <div className="rounded-[2.5rem] border border-white/10 bg-white/5 p-10 backdrop-blur-xl shadow-2xl relative overflow-hidden">
                ...
                <div className="relative flex flex-col md:flex-row md:items-end md:justify-between gap-8">
                  ...
                  <div className="flex items-center gap-3">
                    {/* ✅ BOUTON POUR ALLER À MY PROGRAMS */}
                    <button
                      onClick={() => window.location.href = '/myprograms'}
                      className="flex-1 md:flex-none rounded-2xl border border-[#D6F93D]/30 bg-[#D6F93D]/10 px-6 py-4 text-xs font-black text-[#D6F93D] hover:bg-[#D6F93D]/20 transition flex items-center justify-center gap-2"
                    >
                      <span>VIEW IN MY PROGRAMS</span>
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-[2.5rem] border border-white/5 bg-gradient-to-br from-white/5 to-transparent p-10 relative overflow-hidden">
                    ...
                    <div className="relative">
                      ...
                      <p className="mt-4 text-white/50 text-sm leading-relaxed font-medium italic">
