...
export default function NutritionCatalog({ userId, limit = 4 }) {
  ...
  useEffect(() => {
    ...
    // 🔥 Délai minimal pour éviter les flashes de chargement
  ...
  // 🔥 Mémoïser l'état de chargement
  const isLoading = useMemo(() => {
    return isInitialLoading || meals.length === 0;
  }, [isInitialLoading, meals.length]);

  // 🔥 Empêcher l'affichage pendant le chargement initial
  if (!userId) return null;
